        ...
        switch (src_loc.lazy) {
            ...
            .node_offset_var_decl_ty => |node_off| {
                ...
                const full = switch (tree.nodeTag(node)) {
                    ...
                    .@"usingnamespace" => {
                        return tree.nodeToSpan(tree.nodeData(node).lhs.node);
                    },
                    else => unreachable,
                };
            ...
            .node_offset_ptrcast_operand => |node_off| {
                ...
                while (true) {
                    ...
                    if (tree.nodeData(node).lhs.opt_node == .none) break; // 0 args
                    if (tree.nodeData(node).rhs.opt_node != .none) break; // 2 args
                    ...
                    node = tree.nodeData(node).lhs.opt_node.unwrap().?;
                }
            ...
            .node_offset_array_access_index => |node_off| {
                const tree = try src_loc.file_scope.getTree(gpa);
                const node = node_off.toAbsolute(src_loc.base_node);
                return tree.nodeToSpan(tree.nodeData(node).rhs.node);
            },
            ...
            .node_offset_field_name => |node_off| {
                ...
                const tok_index = switch (tree.nodeTag(node)) {
                    .field_access => tree.nodeData(node).rhs.token,
                    .call_one,
                    .call_one_comma,
                    .async_call_one,
            ...
            .node_offset_asm_ret_ty => |node_off| {
                ...
                return tree.nodeToSpan(tree.nodeData(asm_output).lhs.node);
            },
            ...
            .call_arg => |call_arg| {
                ...
                const call_full = tree.fullCall(buf[0..1], node) orelse {
                    ...
                    const call_args_node: Ast.Node.Index = @enumFromInt(tree.extra_data[@intFromEnum(tree.nodeData(node).rhs.extra_index) - 1]);
            ...
            .node_offset_switch_special_prong => |node_off| {
                ...
                const extra = tree.extraData(tree.nodeData(switch_node).rhs.extra_index, Ast.Node.SubRange);
            ...
            .node_offset_anyframe_type => |node_off| {
                ...
                return tree.nodeToSpan(tree.nodeData(parent_node).rhs.node);
            },
                    ...
                    .range => {
                        ...
                        for (case.ast.values) |item_node| {
                            ...
                            return switch (src_loc.lazy) {
                                ...
                                .switch_case_item_range_first => tree.nodeToSpan(tree.nodeData(item_node).lhs.node),
                                .switch_case_item_range_last => tree.nodeToSpan(tree.nodeData(item_node).rhs.node),
                                else => unreachable,
                            };
                        } else unreachable;
                    },
